...
func (stream * Stream) player_stop() {
	...
	if (stream.state == ST_Stopped) || (stream.state == ST_Stopping) { 
		return
	}
	...
	stream.state_change(ST_Stopping, nil)
}
...
func (stream * Stream) player_start() {
	...
	stream.state_change(ST_Starting, nil)
}
